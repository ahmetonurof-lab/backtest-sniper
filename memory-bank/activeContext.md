...
## Current State
2026-08-07: **TRAIL_MODE default'u `retrace` — düzeltildi (commit `90f0939`).** `5cfa2a3`'te default `continuation` yapılmıştı; bu, normal analyzer koşularını (replay dışı) bozdu: **tüm coin'ler eksi PnL** üretiyordu (doğrulama: ALGOUSDT −58819 / ADAUSDT −57839 vs retrace'te +59140/+31317). Fix: default `retrace` (eski davranış, `fvg_confirm_mode` yalnız `"retrace"` kabul eder, is_placeable bu modda uygulanmaz) — replay modları (`replay_trailing_v2.py:57` `analyzer_v5.TRAIL_MODE = mode`) bundan etkilenmez, A/B/C karşılaştırması korunur. 30 coin replay koşusu hâlâ bekliyor.
