...
- **Canlı doğrulama (G1 pratikte):** Her restart'ta `[RST] BIAS_LOCKED RESTORE` ×10 (bias latch'ler korundu), 28 coin leverage OK, **0 traceback/CRITICAL**. `_inverted_candidates` yeni RSM ile temiz başladı. Aktif pozisyon yoktu — restart güvenli. Yedekler: `output/trade_state.json.pre_ifvg`, `output/active_fvg.json.pre_ifvg`.
- **İzleme eklemesi (sniper f50f633):** Canlı loglama IFVG'yi ayırt edemiyordu (trades_history'de source yok) → `ActiveTrade.entry_source` (NORMAL|IFVG) + bot.py `[IFVG]` entry logu + `tools/ifvg_paper_monitor.py`. Günlük izleme: sunucuda `python3 tools/ifvg_paper_monitor.py` — IFVG entry sayısı/PnL, backtest ~%19 oranı ile karşılaştırma.
- **Anomali kuralı:** beklenmeyen hata / aşırı trade sıklığı / sessiz state tutarsızlığı → flag anında kapat (env unset + restart) + Baş Mühendise rapor.

---

## ✅ IFVG PAPER-DEPLOY G1+G2 — restart persistence belgelendi + NORMAL suppression kök nedeni teşhis edildi; GÖREV 3 BEKLEMEDE (2026-08-18)
- **Direktif:** `ifvg-paper-deploy-on-direktif.md` — 3 görev; G1 (restart persistence) ve G2 (NORMAL suppression teşhisi) paralel yürütüldü, G3 (paper açılışı) G1+G2 raporları onaylanmadan yapılmadı.
- **G1 — Restart/Recovery persistence (`_inverted_candidates`): BUG YOK, bilinçli tasarım kararı (belgelendi).** Kanıt: (1) `recovery_manager.py` RSM'e SIFIR referans — yalnızca Binance pozisyonları → ActiveTrade rebuild; (2) `bot.py::PaperTrader.__init__` her sembol için YENİ `RetraceStateMachine(max_wick_ratio=...)` → `_inverted_candidates=[]` (bellek state'i yok olur); (3) persist edilen tek RSM state'i günlük BIAS latch (`state_manager.py` trade_state.json: daily_bias/sweep_direction/sweep_level/bias_lock_day/bias_lock_bar_index) → `_restore_bias_latch` → `restore_bias_lock()`; (4) `active_fvg.json` yalnızca exit-lifecycle FVG'si. **tick_size bug'ı deseni (kısmi restore eksikliği) YOK** — RSM tamamen sıfırdan kuruluyor, alan asla yarım yüklenmiyor. Etki: restart anında izlenen IFVG adayları kaybolur (kısa ömürlü — kritik değil), bias latch korunur. Beklenen paper davranışı: restart sonrası `_inverted_candidates` boş + `[RST] BIAS_LOCKED RESTORE` + adaylar yeniden birikir. Test: `sniper/tests/test_retrace_state.py::test_restart_simulation_loses_inverted_candidates` → **83/83 PASS**. Rapor: `sniper/reports/ifvg_restart_persistence_bulgu.md`.
- **G2 — NORMAL suppression kök nedeni (3 coin örneklem, bar-bazlı RSM izi):** Araç `tools/diag_ifvg_normal_suppression.py` (monkeypatch `analyzer_v5.process_sweep`, üretim koduna dokunmaz) ARBUSDT/SEIUSDT/XRPUSDT'i IFVG off+on koşar.
  - **Mekanizma A (ana):** body-broken NORMAL FVG `_register_inverted()` ile kaydedilince RSM BIAS_LOCKED'de kalır; flipped aday retest beklerken RSM **IDLE'ye dönemez** → `process_sweep`'in `IDLE+on_sweep` dalı çalışmaz → yeni gün/pencerenin NORMAL sweep→FVG zinciri bastırılır. BLOCKED bar: ARB 948 / SEI 454 / XRP 451 (baskın BIAS_LOCKED).
  - **Mekanizma B (ikincil):** IFVG trigger'ı RSM'i yön flipli BIAS_LOCKED'e sokar → bias_conflict reset'i kilit düşürür (STATE_DIVERGE `IDLE vs BIAS_LOCKED` deseni).
  - **Ölçüm:** SUPPRESSED (NORMAL trigger kaybı) 197/204/106; USURPED (aynı barda IFVG'in NORMAL'i ezmesi) yalnızca 1'er — kayıp filtrelerden değil, state dinamiğinden. ΔNORMAL −213/−222/−107 vs IFVG +353/+396/+233 → **net PnL +7,530/+19,786/+3,937, 3/3 pozitif**.
  - **Karar önerisi: kabul edilebilir trade-off — dokunmaya değmez** (IFVG katkısı NORMAL kaybının ~3.4 katı; 28-coin koşusunda +406,592 vs −119,307). Olası iyileştirme (ayrı iş, paper'ı bloklamaz): IFVG izlemesini ana RSM'den ayırıp paralel mini-state machine → bastırılan NORMAL trade'lerin bir kısmı geri kazanılabilir, ama tetik semantiği değişeceği için 28-coin yeniden doğrulama gerekir. **Direktif gereği KOD DEĞİŞİKLİĞİ YAPILMADI.** Rapor: `reports/ifvg_normal_suppression_diagnoz.md`.
- **G3 — Paper açılışı: YAPILMADI (beklemede).** `IFVG_ENABLED=True` paper config'inde ayarlanmadı; canlı config'e de dokunulmadı. G1+G2 raporları Baş Mühendise sunulduğunda açılabilir — bu oturumda kırmızı çizgi korundu (flag aktif değil).
...
- **Sonnet test revizyonu (3 netleştirme + 1 küçük not) — TAMAMI ELE ALINDI:**
  ...
  2. **R-multiple 15m CLOSE kuralı (Bölüm 5) artık test ediliyor:** `_upnl_r_close` hesabı motor döngüsünden `_sf_unrealized_r_close(s2, cur_close, entry_price, rpt2)` saf fonksiyonuna çıkarıldı (motor `cur.close` geçer — tek kaynak, test+motor ayrı drifte giremez). Yeni `StructuralFallbackCloseRTests` (4 test): "high=+1.8R spike ama close=+1.2R → 1.5R kademesi AKTİF OLMAZ" (`compute_structural_ladder_sl("long", 1.2, 1.0, 10.0)` → step 1.0 BE+fees; 1.8 → step 1.5/10.5), short simetrisi, rpt2=0 → 0.0.
  3. **Smoke `ENABLE_FALLBACK_LADDER=False` netleştirildi:** flag ESKİ merdiven deneyi kalıntısı (analyzer ~1319); SF ladder yalnızca `STRUCTURAL_FALLBACK_MODE`'den beslenir, ondan BAĞIMSIZ (SF bloğu `_sf_gate` koşuluyla). Smoke setUp'ından çıkarıldı + açıklama yorumu eklendi; test şimdi `sf_trail_ladder_count > 0` (HYBRID ladder tarafı egzersiz edildi) ve `_sf_unrealized_r_close` spy (motor close-bazlı tek kaynağı gerçekten çağırdı) da doğruluyor.
  - **Küçük not:** Ladder ratchet "geriye gitmez" kuralı test edildi (`test_ladder_ratchet_never_goes_backward`: 3R'de kilitlenen SL 103.0 varken R=1.3'te 1R BE adayı daha kötü → None).
- **Görev:** Mevcut baseline FVG trailing'i bit-bit koruyarak, FVG bulunamadığında devreye giren STRUCTURAL FALLBACK katmanı eklemek. Varyantlar A/B/C + LUNA madde 9/12/14/15 kısıtları (entry/sweep/MSS/FVG detection/quality/expiry/CBDR/risk/TP_RR değişmez; 1m stratejik trailing yok; no early profit gate; no partial TP). **Kaynak:** `reports/Yerel ajan direktif fallback trailing.md` (eski direktifin YERİNE geçti).
- **FINAL SPEC farkları (Sonnet 2 eklemesi, hepsi uygulandı):**
  - **GATE modeli (madde 1):** `_sf_gate(mode, rpt2, fvg_present)` = `mode is not None and rpt2 > 0 and not fvg_present` — FVG varsa fallback HİÇ hesaplanmaz (competition yok). Saf fonksiyon, birim testli.
  - **SWING producer (madde 5):** `_sf_pick_fallback_candidate` içinde `_SWING_TRAIL_TM._default_level_from_swings(b15[entry_bar:sb+1], s2)` — canlı `TrailingManager` fonksiyonuna DOĞRUDAN çağrı, yeniden implementasyon YOK (iki swing tanımı yasak). `_SWING_TRAIL_TM = TrailingManager(price_reader=None, protection_gateway=None)` (yapıcı güvenli). İmport: `sys.path.insert(0, sniper/src/trading)` + `from trailing_manager import TrailingManager` (paket `trading/__init__.py` ağır importlar içerir — paket üzerinden YASAK). Eski pivot-tablo swing lookup'ı (pivot.find_swing_lows/highs) SF yolu için KALDIRILDI (koşul artık `if PROFIT_PROTECT_GATE_R > 0:`).
  - **Karşılaştırma raporu (madde 8):** `run_compare_sf` artık **5 config**: BASELINE / OLD_PROFIT_GATE_1R (historical `profit_gate=1.0`, TRAIL_BE_ON_GATE=False) / A_LADDER_ONLY / B_SWING_ONLY / C_HYBRID. "## Madde 8: Zorunlu karsilastirma" bölümü OLD vs A_LADDER_ONLY vs BASELINE (NetPnL/PF/Exp$/PTrail%/Loss%/MaxDD/Ladder act./Fallback-only). Her çağrı `structural_fallback_mode` + `profit_gate` explicit (gate sızıntısı yok).
  - **Rejected candidate state'e yazılmaz (madde 2)** — helper saf fonksiyon, (None, None, None) döner; uygulama yalnızca non-None'da. **R = abs(initial_sl − entry_price)**.
- **Mekanizmalar (`analyzer_v5.py`):**
  - `STRUCTURAL_FALLBACK_MODE` (None default = baseline birebir; "LADDER"/"SWING"/"HYBRID"): FVG loop sonrası, atr_chase öncesi; kapı `_sf_gate(STRUCTURAL_FALLBACK_MODE, rpt2, _fvg_present)` (satır ~1361).
  - **LADDER (A; C'de 1-2R):** 15m CLOSE unrealized R ratchet — `SF_LADDER_STEP_1_R=1.0`→BE+fees (round-trip komisyon `entry*(1±c)/(1∓c)`, c=0.0005), `1.5→+0.5R`, `2.0→+1.0R`, `3.0→+1.5R`; intrabar tetiklemez.
  - **SWING (B; C'de R>=2.0):** canlı `_default_level_from_swings` + `SWING_TRAIL_BUFFER(0.10)*ATR` offset; no-lookahead entry_bar sonrası scope + pivot_strength=2 onayı (canlı konfig).
  - **Candidate birleştirme (madde 8):** LONG→max / SHORT→min; TMM min-move (helper `cfg.TRAIL_MIN_MOVE_MULT` okur — fonksiyon-local `TMM` helper'da NameError veriyordu, fix); SL geriye gitmez; TP `abs(delta)` kadar paralel ötelenir.
  - **Raporlama (madde 14):** per-trade `trail_source` (FVG|LADDER|SWING|kombolar|NONE), `trail_candidate/reason/r`, `trail_fvg|ladder|swing`, `trail_sl_before|after`; `_finalize_trail_metrics()`. **Madde 12:** `reached_1pct/1_5pct/2pct/3pct` (bilgi). Özet: `sf_trail_fvg/ladder/swing_count`, `sf_no_trail_count`, `sf_rescued_count`, `sf_fallback_only_(trades|pnl)`, `sf_fvg_only_(trades|pnl)`, `sf_reached_*`.
  - **CLI:** `--trail-exp SF_LADDER_ONLY|SF_SWING_ONLY|SF_HYBRID` + `--compare-sf` (5 config → `reports/analyzer_v5_sf_compare.md`). Worker `_analyze_one_sym_v5` sonuna `structural_fallback_mode`+`swing_trail_buffer`; main() global bildiriminde (serial yol kritik).
- **Yeni test dosyası `tests/test_structural_fallback.py` (28 test, madde 11):** gate truth table (FVG varsa False); swing producer canlı fonksiyonu direkt çağırır (mock + real offset long/short); ladder 4 kademe + <1R none + ratchet; HYBRID exclusive geçiş (1-2R→LADDER, >2R→SWING); rejection (csl'den kötü → None, TMM min-move fail → None, saf/pure + state mutasyonu yok); `_sf_apply_candidate` gerçek atomik apply (3 test); `_sf_unrealized_r_close` 15m CLOSE kuralı (4 test); engine smoke BNBUSDT HYBRID (real feather, `_default_level_from_swings` + `_sf_unrealized_r_close` spy call_count > 0, `sf_trail_ladder_count > 0`). BTCUSDT evrende YOK (Sonnet doğruladı — 28 coin listesinde BTC hiç olmamış), smoke BNBUSDT kullanıyor.
- **SMOKE (BNBUSDT):** baseline 1198/+24,130; A 1195/+22,733 (ladder 112, rescued 93, fallback-only 98/+845); B 1198/+24,171 (swing 1); C 1197/+22,936 (ladder 101, swing 3, rescued 76, fallback-only 87/+844, no-trail 441). Tek-coinde A/C baseline altında; **28 sembol belirleyici**.
- **Doğrulama:** py_compile OK, **67/67 test PASS** (39 eski + 28 yeni). `run_compare_sf` 1-sembol seri koşuda rapor üretti (tuple-order bug fix'li). Sonnet revizyonu commit `24d5cf7`'den SONRA geldi → bu round'da ayrı commit.
- **KOŞU (kullanıcı terminalinde):** `python src\analyzer_v5.py --workers 6 --compare-sf` (28 sembol × 5 config — artık OLD_PROFIT_GATE_1R dahil). Baseline referans: 48,943 / +1,602,063 / PF 3.18 / Exp +32.73; OLD_PROFIT_GATE_1R referans +59,833.
