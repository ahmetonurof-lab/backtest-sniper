...
## 🎯 STRUCTURAL FALLBACK TRAILING (LUNA FİNAL SPEC) — IMPLEMENTE EDİLDİ, 60/60 TEST PASS (2026-08-16, branch main)
...
- **Yeni test dosyası `tests/test_structural_fallback.py` (21 test, madde 11):** gate truth table (FVG varsa False); swing producer canlı fonksiyonu direkt çağırır (mock + real offset long/short); ladder 4 kademe + <1R none; HYBRID exclusive geçiş (1-2R→LADDER, >2R→SWING); rejection (csl'den kötü → None, TMM min-move fail → None, saf/pure + state mutasyonu yok, SL+TP atomik); engine smoke BNBUSDT HYBRID (real feather, `_default_level_from_swings` spy call_count > 0). BTCUSDT evrende YOK (Sonnet doğruladı — 28 coin listesinde BTC hiç olmamış), smoke BNBUSDT kullanıyor.
...
- **Doğrulama:** py_compile OK, **60/60 test PASS** (39 eski + 21 yeni). `run_compare_sf` 1-sembol seri koşuda rapor üretti (tuple-order bug fix'li).
