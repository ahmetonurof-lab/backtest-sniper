...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "sniper", "src"))
os.environ["SNIPER_OUTPUT_DIR"] = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "output")
...
tp_wins = sum(1 for f in entered if f.get("v4_real_result") == "TP" and f.get("v4_real_pnl_usd", 0) > 0)
sl_wins = sum(1 for f in entered if f.get("v4_real_result") == "SL" and f.get("v4_real_pnl_usd", 0) > 0)
