...
df["fvg_bin"] = df["fvg_bin"].cat.rename_categories({
    c: ["Abs_XS","Abs_S","Abs_M","Abs_L"][i] if i < 4 else c
    for i, c in enumerate(df["fvg_bin"].cat.categories)
}) if hasattr(df["fvg_bin"], "cat") and df["fvg_bin"].cat.categories is not None else df["fvg_bin"]
...
# Analizler
fvg_abs = df.groupby(["symbol","fvg_bin"],observed=True).agg(
    Trades=("is_cashflow_positive","count"),Cashflow_WR=("is_cashflow_positive","mean"),
    Avg_R=("r_multiple","mean"),Total_PnL=("final_pnl_usd","sum")).round(3)
fvg_rel = df.groupby(["symbol","rel_fvg_bin"],observed=True).agg(
    Trades=("is_cashflow_positive","count"),Cashflow_WR=("is_cashflow_positive","mean"),
    Avg_R=("r_multiple","mean"),Total_PnL=("final_pnl_usd","sum")).round(3)
side_bias = df.groupby(["symbol","side"]).agg(
    Trades=("is_cashflow_positive","count"),Cashflow_WR=("is_cashflow_positive","mean"),
    Avg_R=("r_multiple","mean")).round(3)
trail_eff = df.groupby(["symbol","trailing_count"]).agg(
    Trades=("is_cashflow_positive","count"),Cashflow_WR=("is_cashflow_positive","mean"),
    Avg_R=("r_multiple","mean")).round(3)
...
print("="*80); print("  ANALIZ A: MUTLAK FVG BOYUTU"); print("="*80)
print(fvg_abs.to_string())
print("\n"+"="*80); print("  ANALIZ B: GORECELI FVG (FVG/ATR)"); print("="*80)
print(fvg_rel.to_string())
print("\n"+"="*80); print("  ANALIZ C: LONG/SHORT ASIMETRISI"); print("="*80)
print(side_bias.to_string())
print("\n"+"="*80); print("  ANALIZ D: TRAILING VERIMLILIGI"); print("="*80)
print(trail_eff.to_string())

print("\n"+"="*80); print("  OZET BULGULAR"); print("="*80)
toxic = fvg_abs[(fvg_abs["Cashflow_WR"]<0.50)&(fvg_abs["Total_PnL"]<0)]
print("\n  Zehirli FVG (WR<%50 ve PnL<0):")
if len(toxic):
    for (sym,bin_),row in toxic.iterrows():
        print(f"    {sym:<10} {bin_:<10} WR={row['Cashflow_WR']:.1%} PnL={row['Total_PnL']:>+8,.0f}")
else:
    print("    (yok)")
...
for sym in sorted(df["symbol"].unique()):
    ...
    if "LONG" in sd.index and "SHORT" in sd.index:
        ...
        if abs(diff)>0.05:
            print(f"    {sym:<10} L={sd.loc['LONG','Cashflow_WR']:.1%} S={sd.loc['SHORT','Cashflow_WR']:.1%} F={diff:+.1%}")
