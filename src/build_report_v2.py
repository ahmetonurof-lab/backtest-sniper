...
with open(dump, "rb") as f:
    data = pickle.load(f)
print(f"Yuklendi: {len(data['all_coin_data'])} coin, {len(data['results_data'])} result", flush=True)
...
try:
    ...
    with open(os.path.join(os.path.dirname(__file__), "..", "reports", "_v5_crash.log"), "w") as ef:
