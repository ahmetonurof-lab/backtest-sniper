...
LRT = 0.003; MFA = 3; DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
...
def ld(fp):
    ...
    with open(fp, encoding="utf-8") as f:
        ...
        for i, row in enumerate(r):
            ...
            b.append(Bar(index=i, open=float(row["open"]), high=float(row["high"]),
...
def run_bt(sym, cfg, use_filter=False):
    ...
    mfs=cfg["min_fvg_size"]; ic=cfg["initial_capital"]; rpt=cfg["risk_per_trade"]
    rp=cfg.get("risk_primary",rpt); rr2=cfg.get("risk_retrade",rpt)
    sam=cfg["sl_atr_mult"]; tpr=cfg["tp_rr"]; fbm=cfg["fvg_buffer_mult"]
    W=500
    flt = CBDR_FILTERS.get(sym, None) if use_filter else None
    b1=ld(fp); b15=r15(b1)
    if not b15: return None,[]
    ss=SessionState(); rsm=RetraceStateMachine(min_fvg_size=mfs)
    rsm2=RetraceStateMachine(min_fvg_size=mfs*0.3)
    trades=[]; active=[]
    skip_today=False; bias_disabled=False
    zone_counts={"golden":0,"gray":0,"red":0}

    for sb in range(W, len(b15)):
        chunk=b15[sb-W:sb+1]; cur=b15[sb]
        atr=max(cur.range, cur.close*0.0001)
        try: edt=datetime.fromtimestamp(cur.timestamp/1000, tz=timezone.utc)
        except: continue
        ...
        if ss.sweep_confirmed and rsm.state_name=="IDLE":
            rsm.on_sweep(direction=ss.sweep_direction or "bullish", level=ss.sweep_level or 0.0, bar_index=cur.index)
        if rsm.state_name=="SWEEP_DETECTED":
            rsm.on_sweep_confirmed(chunk, cur)
        if rsm.can_trigger() and not active:
            sd=rsm.direction; db=ss.daily_bias
            # Bias lock: skip if filter disabled it
            if not bias_disabled:
                if (sd=="bullish" and db==DailyBias.BEARISH) or (sd=="bearish" and db==DailyBias.BULLISH) or db==DailyBias.NEUTRAL:
                    rsm.reset(); continue
            if detect_phase_from_timestamp(cur.timestamp) not in (SessionPhase.NEWYORK, SessionPhase.LONDON):
                rsm.reset(); continue
            side="long" if sd=="bullish" else "short"; ep=cur.close; rp2=atr*sam; tf=rsm.trigger_fvg
            if side=="long":
            ...
            rd=abs(sl-ep)
            if rd<atr*0.1: continue
            qty=(ic*rp)/rd if rd>0 else 0
            if qty<=0: rsm.reset(); continue
            d=datetime.fromtimestamp(cur.timestamp/1000, tz=timezone.utc).strftime("%Y-%m-%d")
            active.append({"entry_bar":sb,"entry_price":ep,"sl":sl,"tp":tp,"qty":qty,"side":side,
                           "trigger_fvg":tf,"initial_sl":sl,"initial_tp":tp,"trailing_count":0,
                           "is_retrade":False,"date":d})
            ss.trades_today+=1; rsm.reset()

        if active and cur.is_closed:
            cfgs=detect_fvgs(chunk, lookback=min(50,len(chunk)), timeframe="15m", min_fvg_size=mfs)
            for t in active:
                if t.get("closed"): continue
                for f in cfgs:
                    if (t["side"]=="long" and f.direction!="bullish") or (t["side"]=="short" and f.direction!="bearish"): continue
                    if f.filled or f.invalidated: continue
                    buf=abs(t["initial_sl"]-t["entry_price"])*fbm
                    if t["side"]=="long":
                        ns=f.bottom-buf
                        if ns>t["sl"]: sd2=ns-t["sl"]; t["sl"]=ns; t["tp"]+=sd2; t["trailing_count"]+=1
                    else:
                        ns=f.top+buf
                        if ns<t["sl"]: sd2=t["sl"]-ns; t["sl"]=ns; t["tp"]-=sd2; t["trailing_count"]+=1
        ...
        for t in active:
            ...
            if ex:
                ...
                trades.append(t)
                if not t.get("is_retrade") and ss.trades_today==1 and not ss.retrade_armed:
                    ss.retrade_armed=True; ss.retrade_side="short" if t["side"]=="long" else "long"
                    ss.retrade_sweep_level=0.0; ss.retrade_entry_bar=t["entry_bar"]
                    ss.retrade_fvg_attempts=0; ss.retrade_mode="fvg"
            else: sa.append(t)
        active=sa

        # retrade (same as original)
        if ss.retrade_armed and ss.trades_today==1 and not active:
            sbi=None; sf=False; lb=min(5,sb)
            for ci in range(max(0,sb-4),sb+1):
                if ci<0 or ci>=len(b15): continue
                if ci-lb<0: continue
                rb=b15[ci-lb:ci]; cb=b15[ci]
                if ss.retrade_side=="short":
                    rh=max(x.high for x in rb)
                    if cb.high>rh and cb.close<rh: sf=True; sbi=ci; break
                else:
                    rl=min(x.low for x in rb)
                    if cb.low<rl and cb.close>rl: sf=True; sbi=ci; break
            if sf:
                sdir="bearish" if ss.retrade_side=="short" else "bullish"
                sb2=b15[sbi]
                if rsm2.state_name=="IDLE": rsm2.on_sweep(direction=sdir, level=ss.retrade_sweep_level, bar_index=sb2.index)
            if sf and rsm2.state_name=="SWEEP_DETECTED":
                sb2=b15[sbi]; sc=b15[sbi-W:sbi+1] if sbi>=W else chunk
                rsm2.on_sweep_confirmed(sc, sb2)
            if rsm2.can_trigger():
                if sbi is not None and sbi<=(ss.retrade_entry_bar or 0): rsm2.reset()
                elif detect_phase_from_timestamp(cur.timestamp) not in (SessionPhase.NEWYORK, SessionPhase.LONDON): rsm2.reset()
                else:
                    rep=cur.close; rrp=atr*sam; rf=rsm2.trigger_fvg
                    if ss.retrade_side=="long":
                        rsl=rf.bottom-(rrp*fbm) if rf else rep-rrp*2
                        rtp=ss.london_high if ss.london_high>rep else rep+rrp*tpr
                    else:
                    ...
                    if rq>0:
                        d2=datetime.fromtimestamp(cur.timestamp/1000, tz=timezone.utc).strftime("%Y-%m-%d")
                        active.append({"entry_bar":sb,"entry_price":rep,"sl":rsl,"tp":rtp,"qty":rq,
                                       "side":ss.retrade_side,"trigger_fvg":rf,"initial_sl":rsl,"initial_tp":rtp,
                                       "trailing_count":0,"is_retrade":True,"date":d2})
                        ss.trades_today+=1; rsm2.reset(); ss.retrade_armed=False
                    else: ss.retrade_fvg_attempts+=1; rsm2.reset()
            else: ss.retrade_fvg_attempts+=1

            if ss.retrade_fvg_attempts>=MFA:
                ss.retrade_mode="lhr"; lh=ss.london_high; ll2=ss.london_low
                if ss.retrade_side=="short" and lh>0:
                    zb=lh*(1-LRT); zt=lh
                    if zb<=cur.close<=zt:
                        lep=cur.close; lsl=lh+atr*1.0
                        ...
                        if lrd>=atr*0.1:
                            ...
                            if lq>0:
                                d2=datetime.fromtimestamp(cur.timestamp/1000, tz=timezone.utc).strftime("%Y-%m-%d")
                                active.append({"entry_bar":sb,"entry_price":lep,"sl":lsl,"tp":ltp,"qty":lq,
                                               "side":"short","trigger_fvg":None,"initial_sl":lsl,"initial_tp":ltp,
                                               "trailing_count":0,"is_retrade":True,"date":d2})
                                ss.trades_today+=1; ss.retrade_armed=False
                elif ss.retrade_side=="long" and ll2<float("inf"):
                    zt=ll2*(1+LRT); zb=ll2
                    if zb<=cur.close<=zt:
                        lep=cur.close; lsl=ll2-atr*1.0
                        ...
                        if lrd>=atr*0.1:
                            ...
                            if lq>0:
                                d2=datetime.fromtimestamp(cur.timestamp/1000, tz=timezone.utc).strftime("%Y-%m-%d")
                                active.append({"entry_bar":sb,"entry_price":lep,"sl":lsl,"tp":ltp,"qty":lq,
                                               "side":"long","trigger_fvg":None,"initial_sl":lsl,"initial_tp":ltp,
                                               "trailing_count":0,"is_retrade":True,"date":d2})
                                ss.trades_today+=1; ss.retrade_armed=False
    ...
    if trades:
        ...
        dpk=ic; dm=0.0; run=ic
        for t in trades:
            run+=t["pnl"]; dpk=max(dpk,run)
            dd=(dpk-run)/dpk*100 if dpk>0 else 0; dm=max(dm,dd)
        ...
        pt=[t for t in trades if not t.get("is_retrade")]; rt=[t for t in trades if t.get("is_retrade")]
        m.update({"total_pnl":tp2,"wr":wr,"max_dd":dm,"profit_factor":pf,
                  "primary_trades":len(pt),"retrade_trades":len(rt),
                  "primary_pnl":sum(t["pnl"] for t in pt),"retrade_pnl":sum(t["pnl"] for t in rt)})
    return m, trades
...
def main():
    ...
    for sym in COINS:
        ...
        print(f"Eski: {m1['total_trades']:>4} islem PnL={m1['total_pnl']:>+8.0f} WR={m1['wr']:>4.1f}% "
    ...
    print(h)
    print("  " + "-" * 125)
    total_old_pnl=0; total_new_pnl=0
    for sym in sorted(COINS):
        o=old_results[sym]; n=new_results[sym]
        di=n['total_trades']-o['total_trades']; dp=n['total_pnl']-o['total_pnl']
        dw=n['wr']-o['wr']
        print(f"  {sym:<10} {o['total_trades']:>7} {o['total_pnl']:>+10.0f} {o['wr']:>5.1f}% {o['max_dd']:>5.1f}% {o['profit_factor']:>5.2f}"
              f" {n['total_trades']:>7} {n['total_pnl']:>+10.0f} {n['wr']:>5.1f}% {n['max_dd']:>5.1f}% {n['profit_factor']:>5.2f}"
              f" {di:>+7} {dp:>+10.0f} {dw:>+5.1f}%")
        total_old_pnl+=o['total_pnl']; total_new_pnl+=n['total_pnl']
    print("  " + "-" * 125)
    td=total_new_pnl-total_old_pnl
    print(f"  {'TOPLAM':<10} {'':>7} {total_old_pnl:>+10.0f} {'':>7} {'':>7} {'':>7}"
