...
def run(symbol: str, session_name: str = "REAL_CBDR", sh: dict = None):
    if sh is None: sh = {'start': 19, 'end': 1}
    csv_path = os.path.join(os.path.dirname(__file__), "data", "daily", f"{symbol}_1m_raw.csv")
    if not os.path.isfile(csv_path): print(f"  VERI YOK: {symbol}"); return

    ic, rpt, sam, tpr, fbm = cfg.INITIAL_BALANCE, cfg.RISK_PER_TRADE, cfg.SL_ATR_MULT, cfg.TP_RR, cfg.FVG_BUFFER_MULT
    ATM, TMM, BERM, BESP, FM = cfg.ATR_TRAIL_MULT, cfg.TRAIL_MIN_MOVE_MULT, cfg.BE_RISK_MULT, cfg.BE_SPREAD_PTS, cfg.FVG_MIN_SIZE_ATR_MULT
    ...
    for bar in b15[1:500]:
        ...
        atr_val = update_atr(atr_val if atr_val > 0 else None, tr); prev_close = bar.close

    active = []
    for sb in range(500, len(b15)):
        chunk, cur = b15[sb - 500: sb + 1], b15[sb]
        tr = calculate_true_range(cur, prev_close)
        atr_val = update_atr(atr_val if atr_val > 0 else None, tr); prev_close = cur.close; atr = atr_val
        ...
        if rsm.can_trigger() and not active:
            ...
            if (h >= sh['start'] or h < sh['end']) if sp else (sh['start'] <= h < sh['end']):
                rsm.reset(); continue
            ...
            if side == "long":
                if tf:
                    fh = tf.top - tf.bottom
                    if fh <= 0: sl = ep - rp2 * 2
                    else: sl = tf.bottom - max(fh * 0.10, max(rp2 * 0.1, min(fh * 0.25, rp2 * fbm)))
                else: sl = ep - rp2 * 2
            else:
                if tf:
                    fh = tf.top - tf.bottom
                    if fh <= 0: sl = ep + rp2 * 2
                    else: sl = tf.top + max(fh * 0.10, max(rp2 * 0.1, min(fh * 0.25, rp2 * fbm)))
                else: sl = ep + rp2 * 2
            rd = abs(sl - ep)
            if rd < atr * 0.1 or rd <= 0: rsm.reset(); continue
            tp = ep + rd * tpr if side == "long" else ep - rd * tpr
            qty = (ic * rpt) / rd if rd > 0 else 0
            if qty <= 0: rsm.reset(); continue
            active.append({"entry_bar": sb, "entry_price": ep, "sl": sl, "tp": tp, "qty": qty,
                           "side": side, "initial_sl": sl, "initial_tp": tp, "trailing_count": 0, "day_key": ss.cbdr_day})
            rsm.reset()

        if active and cur.is_closed:
            for t in active:
                if t.get("closed") or t.get("trailing_count", 0) > 0: continue
                rpt2 = abs(t["initial_sl"] - t["entry_price"]); th2 = rpt2 * BERM
                be2 = t["entry_price"] + BESP if t["side"] == "long" else t["entry_price"] - BESP
                if t["side"] == "long":
                    if cur.high >= t["entry_price"] + th2 and t["sl"] < be2: t["sl"] = be2; t["trailing_count"] = 1
                else:
                    if cur.low <= t["entry_price"] - th2 and t["sl"] > be2: t["sl"] = be2; t["trailing_count"] = 1
            ...
            for t in active:
                ...
                rpt2 = abs(t["initial_sl"] - t["entry_price"]); upd = False
                for fvg in cfvgs:
                    if t["side"] == "long" and fvg.direction != "bullish": continue
                    if t["side"] == "short" and fvg.direction != "bearish": continue
                    if not _close(fvg, tc): continue
                    ab2 = atr * ATM
                    if t["side"] == "long":
                        ns = fvg.bottom - ab2
                        if ns > t["sl"] and (ns - t["sl"]) > rpt2 * TMM:
                            sd2 = ns - t["sl"]; t["sl"] = ns; t["tp"] += sd2; upd = True
                    else:
                        ns = fvg.top + ab2
                        if ns < t["sl"] and (t["sl"] - ns) > rpt2 * TMM:
                            sd2 = t["sl"] - ns; t["sl"] = ns; t["tp"] -= sd2; upd = True
                if upd: t["trailing_count"] = t.get("trailing_count", 0) + 1

        sa = []
        for t in active:
            if t.get("closed"): continue; ex = False
            if t["side"] == "long":
                if cur.low <= t["sl"]: t["exit_price"] = t["sl"]; t["result"] = "SL"; t["closed"] = True; ex = True
                elif cur.high >= t["tp"]: t["exit_price"] = t["tp"]; t["result"] = "TP"; t["closed"] = True; ex = True
            else:
                if cur.high >= t["sl"]: t["exit_price"] = t["sl"]; t["result"] = "SL"; t["closed"] = True; ex = True
                elif cur.low <= t["tp"]: t["exit_price"] = t["tp"]; t["result"] = "TP"; t["closed"] = True; ex = True
    ...
    print(f"  {'CBDR%':<12} {'Gun':>4} {'Trade':>6} {'WR%':>6} {'PF':>6} {'AvgR:R':>7} {'PnL':>10}")
    print(f"  {'-'*55}")
...
def _res(b1):
    ...
    for i in range(0, len(b1), 15):
        c = b1[i:i + 15]
        if len(c) < 15: break
        m.append(Bar(index=c[0].index, open=c[0].open, high=max(b.high for b in c),
                     low=min(b.low for b in c), close=c[-1].close, volume=sum(b.volume for b in c),
