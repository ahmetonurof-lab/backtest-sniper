...
def run_backtest(symbol):
    ...
    ATM = cfg.ATR_TRAIL_MULT; TMM = cfg.TRAIL_MIN_MOVE_MULT
    BERM = cfg.BE_RISK_MULT; BESP = cfg.BE_SPREAD_PTS
    FVG_MIN_SIZE_ATR_MULT = cfg.FVG_MIN_SIZE_ATR_MULT
    FVG_WICK_RATIO_MAX = cfg.FVG_WICK_RATIO_MAX

    b1 = load_data(csv_path)
    b15 = resample_15m(b1)
    if not b15: return None, None
    ...
    for sb in range(500, len(b15)):
        ...
        if ss.sweep_confirmed and rsm.state_name == "IDLE":
            rsm.on_sweep(direction=ss.sweep_direction or "bullish", level=ss.sweep_level or 0.0, bar_index=None)
        ...
        if rsm.can_trigger() and not active:
            ...
            ep = cur.close; rp2 = atr * sam; tf = rsm.trigger_fvg
            ...
            if side == "long":
                if tf:
                    ...
                    sl = ep - rp2 * 2
                rd = abs(sl - ep)
                if tf and rd > rp2 * 2.0: sl = ep - rp2 * 2; rd = abs(sl - ep)
                if rd <= 0: sl = ep - rp2 * 2; rd = abs(sl - ep)
                tp = ep + rd * tpr
            else:
                if tf:
                    fh = tf.top - tf.bottom
            ...
            if rd < atr * 0.1: rsm.reset(); continue
            qty_base = (ic * rpt) / rd if rd > 0 else 0
            if qty_base <= 0: rsm.reset(); continue

            active.append({"sb": sb, "ep": ep, "sl": sl, "tp": tp, "qty_base": qty_base, "side": side,
                           "is": sl, "it": tp, "tc": 0, "dk": ss.cbdr_day})
            rsm.reset()

        if active and cur.is_closed:
            for t in active:
                if t.get("cl"): continue
                if t.get("tc", 0) > 0: continue
                s2 = t["side"]; e2 = t["ep"]
                rpt2 = abs(t["is"] - e2); th2 = rpt2 * BERM
                be2 = e2 + BESP if s2 == "long" else e2 - BESP
                if s2 == "long":
                    if cur.high >= e2 + th2 and t["sl"] < be2: t["sl"] = be2; t["tc"] = 1
                else:
                    if cur.low <= e2 - th2 and t["sl"] > be2: t["sl"] = be2; t["tc"] = 1
            ...
            for t in active:
                ...
                rpt2 = abs(t["is"] - t["ep"]); ltc = 0; upd = False
                for fvg in cfvgs:
                    if s2 == "long" and fvg.direction != "bullish": continue
                    if s2 == "short" and fvg.direction != "bearish": continue
                    if not fvg_close_confirmed(fvg, tc): continue
                    ab2 = atr * ATM
                    if s2 == "long":
                        ns = fvg.bottom - ab2
                        if ns > csl and (ns - csl) > rpt2 * TMM:
                            sd2 = ns - csl; csl = ns; ctp += sd2; ltc += 1; upd = True
                    else:
                        ns = fvg.top + ab2
                        if ns < csl and (csl - ns) > rpt2 * TMM:
                            sd2 = csl - ns; csl = ns; ctp -= sd2; ltc += 1; upd = True
                if upd: t["sl"] = csl; t["tp"] = ctp; t["tc"] = t.get("tc", 0) + ltc
        ...
        for t in active:
            ...
            ex = False
            if t["side"] == "long":
                if cur.low <= t["sl"]: t["xp"] = t["sl"]; t["cl"] = True; ex = True
                elif cur.high >= t["tp"]: t["xp"] = t["tp"]; t["cl"] = True; ex = True
            else:
                if cur.high >= t["sl"]: t["xp"] = t["sl"]; t["cl"] = True; ex = True
                elif cur.low <= t["tp"]: t["xp"] = t["tp"]; t["cl"] = True; ex = True
            if ex:
                diff = (t["xp"] - t["ep"]) if t["side"] == "long" else (t["ep"] - t["xp"])
...
def compute_bucket_pnls(day_cbdr, day_pnls, risk_mult):
    ...
    """
    valid = {d: day_cbdr[d] for d in day_cbdr if d in day_pnls and day_cbdr[d] is not None}
    ...
    for bi in range(N_BUCKETS):
        lo = mn + bi * bw; hi = lo + bw
        if bi == N_BUCKETS - 1: hi = mx + 0.001
        bd = [d for d in sd if lo <= valid[d] < hi]
        if not bd: continue
        tl = [p * risk_mult for d in bd for p in day_pnls.get(d, [])]
        if not tl: continue
        n = len(tl); wins = sum(1 for p in tl if p > 0)
        wr = wins / n * 100 if n > 0 else 0; pnl = sum(tl)
        gp = sum(p for p in tl if p > 0) or 0
        gl = abs(sum(p for p in tl if p < 0)) or 1e-9
        buckets.append({'range': f"{lo:.2f}-{hi:.2f}", 'days': len(bd), 'trades': n,
                        'wr': wr, 'pnl': pnl, 'pf': gp/gl, 'rm': risk_mult})
    return buckets
...
def main():
    ...
    print(f"  Session: REAL_CBDR ({SESSION_HOURS['start']}:00-{SESSION_HOURS['end']}:00 UTC)")
    ...
    for sym in sorted(cfg.SYMBOLS):
        ...
        for rm in RISK_MULTIPLIERS: print(f" {'x'+str(rm):>10}", end="")
        print()
    ...
    for k in sorted(ba.keys(), key=lambda x: x[0]):
        _, rng = k; d = ba[k]
        br = None; ba2 = -float('inf')
        print(f"  {rng:<18} ", end="")
        for rm in RISK_MULTIPLIERS:
            vs = d.get(rm, [])
            av = sum(vs) / len(vs) if vs else 0
            if av > ba2: ba2 = av; br = rm
            if vs: print(f" {av:>+9.0f}", end="")
            else: print(f" {'N/A':>10}", end="")
        print(f" {'x'+str(br):<6}")
        ...
        for sym in sorted(all_results):
            for rm in sorted(all_results[sym].keys()):
                for b in all_results[sym][rm]:
                    w.writerow([sym, rm, b['range'], b['days'], b['trades'],
                                f"{b['wr']:.1f}", f"{b['pnl']:.2f}", f"{b['pf']:.2f}"])
    print(f"\n  CSV: {csv_path}")
