...
def question1():
    print("=" * 100)
    print(
        "  SORU 1: Entry Saati Dagitimi (UTC) — Hangi saatlerde gercekten trade acilmis?"
    )
    print("=" * 100)

    for fname in [
        "trades_default.parquet",
        "trades_real_cbdr.parquet",
        "trades_asia_range.parquet",
    ]:
        ...
        for sym in sorted(df["symbol"].unique()):
            ...
            hours_str = ", ".join(
                f"{r['hour']:02d}:00({r['cnt']})" for _, r in top3.iterrows()
            )
...
def question2():
  ...
  """)
    ...
    for sym in ["ADAUSDT", "APTUSDT", "ETHUSDT", "XRPUSDT"]:
        ...
        print(
            f"  {sym:<10} {tot:>6} trade | WIN:{wins:>5} BE:{be:>5} LOSS:{losses:>5} | "
...
def question3():
  ...
  """)
    ...
    for fname in [
        ...
        data[session_key] = {"all": df, "asia": df[df["is_asia"]], "ln": london_ny}

    # Karsilastirma tablosu
    print(
        f"  {'Session':<15} {'Segment':<12} {'Trades':>8} {'Wins':>7} {'WR%':>7} {'AvgPnL':>8}"
    )
    print(f"  {'-'*55}")
    ...
    for sym in sorted(dft["symbol"].unique()):
        ...
        print(
            f"  {sym:<10} {n_a:>8,} {wr_a:>7.1f}% {n_l:>8,} {wr_l:>7.1f}% {wr_a-wr_l:>+7.1f}%"
        )
