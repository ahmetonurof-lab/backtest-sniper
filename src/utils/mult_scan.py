...
def main():
    ...
    for sym in cfg.SYMBOLS:
        ...
        print(
            f"  {sym}: {len(bars_15m):>6,} bars, avg_real_atr={avg_atr:.4f} [{time.time()-t0:.0f}s]",
            flush=True,
        )
    ...
    for mult in MULT_RANGE:
        ...
        for sym in sorted(coin_data.keys()):
            ...
            if m:
                ...
                print(
                    f"MULT={mult:.2f} {sym}: min_fvg={min_fvg:.4f} "
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("**Data:** 2.5 yil (2024-01-01 → 2026-06-30)\n")
        f.write(
            f"**MULT range:** {MULT_RANGE[0]:.2f} – {MULT_RANGE[-1]:.2f}, adim 0.02\n"
        )
        f.write("**Formul:** `min_fvg_size = atr_val × MULT` (dinamik, bar-bazli)\n")
        f.write(
            f"**Ornek (BTC):** MULT=0.02 → ~{0.02*coin_data.get('BTCUSDT',{}).get('avg_atr',0):.1f}, MULT=0.12 → ~{0.12*coin_data.get('BTCUSDT',{}).get('avg_atr',0):.1f}, MULT=0.30 → ~{0.30*coin_data.get('BTCUSDT',{}).get('avg_atr',0):.1f}\n\n"
        )
        ...
        for mult in mults_done:
            ...
            f.write(
                f"| {mult:.2f} | {total_t:,} | {total_p:+,.0f} | {avg_wr:.1f}% | {avg_mdd:.1f}% | {avg_pf:.2f} |\n"
            )
        ...
        for sym in sorted(coin_data.keys()):
            ...
            if best_r:
                ...
                f.write(
                    f"| {sym:<8} | {avg_atr:<8.4f} | {best_mult:.2f} | {mf:<.4f} | {best_r['trades']:<5} | {best_r['wr']:.1f}% | {best_r['pnl']:<+9.0f} | {best_r['mdd']:.1f}% | {best_r['pf']:.2f} |\n"
                )
