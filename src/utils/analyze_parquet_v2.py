...
from session import detect_phase
...
def analyze():
  ...
  """)
    ...
    for fname in [
        ...
        print(f"\n  --- {label} ---")
        print(
            f"  {'Phase':<12} {'Trades':>8} {'Wins':>7} {'WR%':>7} {'AvgPnL':>9} {'TotalPnL':>10}"
        )
        print(f"  {'-'*55}")
    ...
    print("  (Asia = 02-08 kodun RangeTracker.asia siniri, London icin degil)")
    print(
        f"  {'Session':<18} {'AsiaWr':>8} {'EarlyLonWr':>12} {'NY_WR':>8} {'Asia-LonFark':>12}"
    )
    print(f"  {'-'*60}")
    ...
    for fname in [
        ...
        early_lon = df[
            df["hour"].between(8, 12)
        ]  # Londra 13'e kadar ama Asya'dan sonrasi
        ...
        print(
            f"  {label:<18} {a_wr:>7.1f}% {e_wr:>10.1f}% {n_wr:>7.1f}% {a_wr-e_wr:>+10.1f}%"
        )
