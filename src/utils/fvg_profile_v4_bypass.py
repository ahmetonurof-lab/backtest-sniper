"""
fvg_profile_v4_bypass.py — V4 engine BYPASS (A/B). FVG_QUALITY/FVG_VALIDITY/SHOULD_TRADE devre disi.
...
from session_router import get_cbdr_multiplier, get_session_hours
...
def detect_sweep(b15, c3_pos, lookback=20):
    ...
    for i in range(2, len(pre_bars) - 2):
        ...
        if (
            l < pre_bars[i - 2].low
            and l < pre_bars[i - 1].low
            and l < pre_bars[i + 1].low
            and l < pre_bars[i + 2].low
        ):
...
def detect_bos_mss(fvg, b15, hi, lo):
    ...
    pre_closes = (
        [b.close for b in b15[pre_start:c3_idx] if b.index < c3_idx]
        if c3_idx > pre_start
        else []
    )
    ...
    pre_bos = (
        any(idx < c3_idx and pre_max_c > pr for idx, pr in sw_h)
        if trend == "uptrend"
        else (
            any(idx < c3_idx and pre_min_c < pr for idx, pr in sw_l)
            if trend == "downtrend"
            else False
        )
    )
    pre_mss = (
        ...
        if trend == "uptrend"
            ...
            any(idx < c3_idx and pre_max_c > pr for idx, pr in sw_h)
            if trend == "downtrend"
            else any(idx < c3_idx and pre_max_c > pr for idx, pr in sw_h)
            or any(idx < c3_idx and pre_min_c < pr for idx, pr in sw_l)
        )
    )
    post_end = min(c3_idx + 21, len(b15))
    post_closes = (
        [b.close for b in b15[c3_idx + 1 : post_end] if b.index > c3_idx]
        if post_end > c3_idx + 1
        else []
    )
    ...
    wt = (
        "downtrend"
        if trend == "uptrend"
        else ("uptrend" if trend == "downtrend" else "ranging")
    )
    post_bos = (
        any(idx < c3_idx and post_max_c > pr for idx, pr in sw_h)
        if wt == "uptrend"
        else (
            any(idx < c3_idx and post_min_c < pr for idx, pr in sw_l)
            if wt == "downtrend"
            else False
        )
    )
    post_mss = (
        ...
        if wt == "uptrend"
            ...
            any(idx < c3_idx and post_max_c > pr for idx, pr in sw_h)
            if wt == "downtrend"
            else any(idx < c3_idx and post_max_c > pr for idx, pr in sw_h)
            or any(idx < c3_idx and post_min_c < pr for idx, pr in sw_l)
        )
    )
...
def volatility_regime_analysis(fvgs, b15, window=50):
    ...
    for f in fvgs:
        ...
        if (
            f["rr"].get("hit_target")
            or f["rr"].get("hit_stop")
            or f["rr"].get("no_outcome")
        ):
...
def _collect_fvg_profile_impl(symbol: str):
    ...
    for sb in range(500, total_bars):
        ...
        if rsm.can_trigger() and not active:
            ...
            # ── FVG quality filter (BYPASS — A/B testi, is_high_quality_fvg/is_fvg_valid devre disi) ──
            quality_mult = 1.0
            # if tf is not None:
            ...
            # allowed, reason = should_trade(symbol, cbdr_width_pct=cbdr_w)  # BYPASS
            # if not allowed:
            #     quality_mult = 0.0
...
def build_report(all_coin_data, results_data):
    ...
    for sym, coin_data in all_coin_data.items():
        ...
        L(
            f"| {sym:<8s} | {total:>6d} | {entered:>6d} | {fvg_q:>6d} | {fvg_v:>6d} | {min_r:>6d} | {cbdr_r:>6d} | {qty_z:>6d} |"
        )
    L("")
    ...
    for sym, coin_data in all_coin_data.items():
        ...
        L(
            f"| {sym:<8s} | {session_label:<10s} | {best_cat:<12s} | {cfg.GLOBAL_FVG_EXPIRY_BARS:>3d}b | "
...
def main():
    ...
    parquet_path = os.path.join(
        ...
        "..",
        "reports",
        "trades_multi_session_bypass.parquet",
    )
    global _LOGGER
    ...
    md_path = os.path.join(report_dir, "fvg_profile_v4_bypass.md")
