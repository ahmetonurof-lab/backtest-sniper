...
def run_backtest(symbol: str, early_london_risk_mult: float) -> dict:
        ...
        for t in active:
            ...
            if ex:
                ...
                trade_records.append(
                    {"result": t["result"], "pnl": t["pnl"], "hour": t["entry_hour"]}
                )
            else:
                sa.append(t)
        active = sa
    ...
    el_trades = [
        r for r in trade_records if r["hour"] is not None and 2 <= r["hour"] < 8
    ]
    ...
    return {
        ...
        "el_trades": len(el_trades),
        "el_wins": el_wins,
        "el_wr": round(el_wr, 1),
        "el_pnl": round(el_pnl, 2),
    }
...
def main():
    ...
    print(
        f"  Coin sayisi: {len(cfg.SYMBOLS)} × {len(SWEEP_VALUES)} = {len(cfg.SYMBOLS)*len(SWEEP_VALUES)} backtest"
    )
    print("=" * 100)
    ...
    for mult in SWEEP_VALUES:
        ...
        for sym in all_symbols:
            ...
            if stats:
                ...
                print(
                    f"{sym}: {stats['trades']}t WR={stats['wr']}% PnL={stats['total_pnl']:+.0f} DD={stats['max_dd_pct']}%"
                )
            else:
    ...
    md.append(
        "**Early London Window:** 02:00-08:00 UTC (kodun `detect_phase()` LONDON baslangici)"
    )
    md.append("")
    ...
    for mult in SWEEP_VALUES:
        total_pnl = sum(
            results[sym].get(str(mult), {}).get("total_pnl", 0) for sym in all_symbols
        )
        wrs = [results[sym].get(str(mult), {}).get("wr", 0) for sym in all_symbols]
        dds = [
            results[sym].get(str(mult), {}).get("max_dd_pct", 0) for sym in all_symbols
        ]
        el_pnl = sum(
            results[sym].get(str(mult), {}).get("el_pnl", 0) for sym in all_symbols
        )
        el_wrs = [
            results[sym].get(str(mult), {}).get("el_wr", 0) for sym in all_symbols
        ]
        ...
        md.append(
            f"| {mult:.1f}x | {total_pnl:>+9,.0f} | {avg_wr:>5.1f}% | {port_max_dd:>6.2f}% | {pnl_dd_ratio:>6.1f} | {el_pnl:>+9,.0f} | {avg_el_wr:>5.1f}% |"
        )
    ...
    for sym in all_symbols:
        best_mult = max(
            SWEEP_VALUES,
            key=lambda m: results[sym].get(str(m), {}).get("total_pnl", -1e9),
        )
        s = results[sym].get(str(best_mult), {})
        md.append(
            f"| {sym} | {best_mult:.1f}x | {s.get('total_pnl',0):>+8,.0f} | {s.get('wr',0):>4.1f}% | {s.get('max_dd_pct',0):>5.2f}% |"
        )
    ...
    for sym in all_symbols:
        for mult in SWEEP_VALUES:
            ...
            if s:
                md.append(
                    f"| {sym} | {mult:.1f}x | {s['trades']} | {s['wr']}% | {s['total_pnl']:>+,.0f} | {s['max_dd_pct']:.2f}% | {s['el_trades']} | {s['el_wr']}% | {s['el_pnl']:>+,.0f} |"
                )

    md.append("")
    md.append("*CSV: `sweep_early_london.csv`*")
