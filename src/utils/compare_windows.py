...
def main():
            ...
            try:
                ...
                for t in result.get("trades", []):
                    ...
                    all_trade_records.append(
                        (trade_id, sname, t.get("pnl", 0), t.get("result", ""), sym)
                    )
                print(
                    f"    [{sname}] {result.get('total_trades', 0)} islem, WR={result.get('wr', 0):.1f}%",
                    flush=True,
                )
    ...
    for sym in sorted(cfg.SYMBOLS):
        ...
        tee_print(
            f"  │ {'Session':<14} {'Trade':>7} {'WR%':>7} {'PF':>7} {'MaxDD%':>8} {'AvgMAE':>8} {'PnL':>10} │",
            log_fh,
        )
        tee_print(
            f"  ├{'─'*14}┼{'─'*7}┼{'─'*7}┼{'─'*7}┼{'─'*8}┼{'─'*8}┼{'─'*10}┤", log_fh
        )
        for sname in SESSION_CONFIGS:
            ...
            session_unique = [
                r for r in unique_records if r[1] == sname and r[4] == sym
            ]
            ...
            csv_rows.append(
                {
                    "Coin": sym,
                    "Session": sname,
                    "Trades_Raw": len(raw_trades),
                    "Trades_Unique": stats["trades"],
                    "WR": round(stats["wr"], 1),
                    "PF": round(stats["pf"], 2),
                    "MaxDD": round(stats["mdd"], 2),
                    "AvgMAE": round(stats["avg_mae"], 2),
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("**Full log:** `v3_window_comparison_full.txt`\n\n")
        ...
        for row in csv_rows:
            f.write(
                f"| {row['Coin']:<8} | {row['Session']:<10} | {row['Trades_Raw']:>4} | {row['Trades_Unique']:>4} | "
