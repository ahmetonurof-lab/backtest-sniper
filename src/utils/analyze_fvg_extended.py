...
def classify_c3(fvg, c2, c3, atr):
    ...
    if direction == "bullish":
        if (
            c3.close < c3.open
            and c3_body >= atr * REJECTION_ATR_MULT
            and brr >= EXPANSION_BODY_RANGE_RATIO
        ):
            return "REJECTION"
        if (
            c3.close > c3.open
            and c3_body >= atr * EXPANSION_ATR_MULT
            and brr >= EXPANSION_BODY_RANGE_RATIO
            and c3.high > c2.high
        ):
            return "EXPANSION"
    ...
    for offset, b in enumerate(bars_after):
        ...
        if mitigated and result["continuation"] is None:
            ...
            if future < len(bars_after):
                ...
                result["continuation"] = (
                    (fb.close > top) if direction == "bullish" else (fb.close < bottom)
                )
...
def simulate_rr(fvg, outcome, commission=0.0):
    ...
    net_profit = (
        (reward - total_fee) if hit_target else (-risk - total_fee) if hit_stop else 0
    )
...
def bootstrap_ci(records, n_iter=10000, ci=0.95):
    ...
    for cat, items in cats.items():
        ...
        result[cat] = {
            ...
            "mit_ci": (
                mit_samples[int(alpha * n_iter)],
                mit_samples[int((1 - alpha) * n_iter)],
            ),
            "exp_mean": sum(exp_samples) / n_iter,
            "exp_ci": (
                exp_samples[int(alpha * n_iter)],
                exp_samples[int((1 - alpha) * n_iter)],
            ),
        }
    return result
...
def sensitivity_analysis(symbol, hours, resample_n, base_params):
    ...
    for label, exp_mult, rej_mult in variations:
        ...
        old_exp, old_rej, old_brr = (
            EXPANSION_ATR_MULT,
            REJECTION_ATR_MULT,
            EXPANSION_BODY_RANGE_RATIO,
        )
        if label != "base":
        ...
        EXPANSION_ATR_MULT, REJECTION_ATR_MULT, EXPANSION_BODY_RANGE_RATIO = (
            old_exp,
            old_rej,
            old_brr,
        )
        ...
        for r in recs:
            ...
            if r["mitigated"]:
                cats[r["cat"]]["mit"] += 1
        results[label] = {
            c: (d["mit"] / d["total"] * 100 if d["total"] else 0)
    ...
    for sym, path in fvg_dirs.items():
        if os.path.exists(path):
            with open(path) as f:
                data = json.load(f)
            coin_fvgs[sym] = {
                (r["ts"] // 900000, r["direction"]): r for r in data
            }  # 15m buckets
    if len(coin_fvgs) < 2:
        return {}
    # Pairwise Jaccard
...
def print_bootstrap(ci_data):
    print(f"\n{'='*80}")
    print("  1. BOOTSTRAP CI (%95 güven aralığı, n=10000 resample)")
    print(f"{'='*80}")
    print(
        f"  {'Kategori':<16} {'n':>6} {'Mit%':>8} {'%95 CI':>16} {'Exp(R)':>8} {'%95 CI':>16}"
    )
    print(f"  {'-'*70}")
    for cat in ["CONSOLIDATION", "EXPANSION", "REJECTION"]:
        d = ci_data.get(cat, {})
        print(
            f"  {cat:<16} {d.get('n',0):>6} {d.get('mit_mean',0)*100:>7.1f}% "
...
def print_sensitivity(sens_data):
    print(f"\n{'='*80}")
    print("  3. SENSITIVITY ANALYSIS (threshold ±20%, ±30%)")
    print(f"{'='*80}")
    ...
    for label, data in sens_data.items():
        order = ""
        vals = [
            (c, data.get(c, 0)) for c in ["CONSOLIDATION", "EXPANSION", "REJECTION"]
        ]
        vals.sort(key=lambda x: -x[1])
        order = " > ".join([v[0][:4] for v in vals])
        print(
            f"  {label:<12} {data.get('CONSOLIDATION',0):>7.1f}% {data.get('EXPANSION',0):>7.1f}% {data.get('REJECTION',0):>7.1f}% {order:>20}"
        )
...
def print_correlation(corr_data, n_coins):
    ...
    print(
        f"  Effective independent samples (est): {corr_data.get('effective_n', 0):.1f}"
    )


def print_comparison(configs_results):
    print(f"\n{'='*80}")
    print("  5. SESSION/TF KARŞILAŞTIRMASI")
    print(f"{'='*80}")
    print(
        f"  {'Config':<16} {'FVG':>6} {'CONS%':>8} {'EXP%':>8} {'REJ%':>8} {'Exp(R)':>8}"
    )
    print(f"  {'-'*54}")
    for name, recs in configs_results:
        ...
        print(
            f"  {name:<16} {len(recs):>6} {cons_m:>7.1f}% {exp_m:>7.1f}% {rej_m:>7.1f}% {avg_exp:>+7.2f}R"
        )


def print_commission(recs_no_comm, recs_with_comm):
    print(f"\n{'='*80}")
    print("  6. COMMISSION (%0.04 taker) — EXPECTANCY KARŞILAŞTIRMASI")
    print(f"{'='*80}")
    ...
    for cat in ["CONSOLIDATION", "EXPANSION", "REJECTION"]:
        ...
        if g and n:
            print(
                f"  {cat:<16} {sum(g)/len(g):>+9.3f}R {sum(n)/len(n):>+9.3f}R {sum(n)/len(n)-sum(g)/len(g):>+9.3f}R"
            )
...
def main():
    ...
    btc_path = os.path.join(
        os.path.dirname(__file__), "..", "reports", "fvg_data_DEFAULT_15m_BTCUSDT.json"
    )
    ...
    for sym in SYMBOLS_CORE:
        p = os.path.join(
            ...
            "..",
            "reports",
            f"fvg_data_DEFAULT_15m_{sym}.json",
        )
        if os.path.exists(p):
            with open(p) as f:
                d = json.load(f)
    ...
    print(f"\n{'='*80}")
    print("  2. ATR LOOK-AHEAD BIAS CHECK")
    print(f"{'='*80}")
    print("  ATR hesaplamasinda update_atr() her bar icin sadece onceki")
    print("  bar'in true_range'ini kullanir (expand eden window).")
    print("  Kodda: atr_val = update_atr(onceki_atr, current_tr)")
    print("  Yani bar[i]'nin atr'i sadece bar[0..i] verisiyle hesaplanir.")
    print("  ✅ Look-ahead bias YOK — only historical data used.")

    # 3. Sensitivity analysis (BTC only)
    print("\n--- Sensitivity analysis hesaplaniyor (BTCUSDT) ---")
    base_params = {
        "exp_atr": EXPANSION_ATR_MULT,
        "rej_atr": REJECTION_ATR_MULT,
        "brr": EXPANSION_BODY_RANGE_RATIO,
    }
    sens_data = sensitivity_analysis(
        "BTCUSDT", CONFIGS[0]["hours"], CONFIGS[0]["resample"], base_params
    )
    ...
    print(f"\n{'='*80}")
    print("  RAPOR: reports/fvg_3rd_candle_extended_report.md")
    print(f"{'='*80}")
