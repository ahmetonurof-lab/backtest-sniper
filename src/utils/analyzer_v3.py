...
def run_for_symbol(symbol: str, session_hours: dict = None):
    ...
    if session_hours:
        ss = SessionState(
            start_hour=session_hours["start"], end_hour=session_hours["end"]
        )
    else:
    ...
    for scan_bar in range(WINDOW, len(bars_15m)):
                ...
                if trigger_fvg:
                    ...
                    if fvg_height <= 0:
                        ...
                        adaptive_buf = max(
                            ...
                            max(
                                risk_pts * 0.1,
                                min(fvg_height * 0.25, risk_pts * fvg_buffer_mult),
                            ),
                        )
        ...
        if active_trades and current.is_closed:
            for trade in active_trades:
                ...
                be_sl = (
                    entry + BE_SPREAD_PTS_V3
                    if side == "long"
                    else entry - BE_SPREAD_PTS_V3
                )
                if side == "long":
            ...
            current_fvgs = detect_fvgs(
                trail_chunk,
                lookback=min(50, len(trail_chunk)),
                timeframe="15m",
            ...
            for trade in active_trades:
                ...
                for fvg in current_fvgs:
                    ...
                    if side == "short" and fvg.direction != "bearish":
                        continue
                    if not fvg_close_confirmed(fvg, trail_chunk):
                        continue
                    ...
                    if side == "long":
                        ...
                        if (
                            ...
                            > risk_pts_t * TRAIL_MIN_MOVE_MULT_V3
                        ):
                ...
                if updated:
                    ...
                    trade["trailing_count"] = (
                        trade.get("trailing_count", 0) + local_trail_count
                    )
                    pipeline["trailing_sl_updates"] += 1
    ...
    print(
        f"                FVG buffer={fvg_buffer_mult}x risk_pts | Session=NY+LON | Unlimited entry"
    )
    ...
    if trades:
        ...
        avg_trailing = (
            sum(t.get("trailing_count", 0) for t in trades) / len(trades)
            if trades
            else 0
        )
        ...
        if wt > 0 and lt != 0:
            print(f"  {'Profit Factor (W/L)':<30}{abs(wt/lt):.2f}")
        ...
        print(
            f"  {'SHORT':<12}{len(short_t):<8}{swr:<7.1f}%{spnl:<+14.2f}{sawr:<+12.2f}"
        )
        ...
        if trailed and not_trailed:
            print(f"\n  TRAILING ETKISI")
            print(f"  {'-'*56}")
            tpnl = sum(t["pnl"] for t in trailed)
            npnl = sum(t["pnl"] for t in not_trailed)
            twr = sum(1 for t in trailed if t["pnl"] > 0) / len(trailed) * 100
            nwr = sum(1 for t in not_trailed if t["pnl"] > 0) / len(not_trailed) * 100
            print(
                f"  {'Trailing aktif':<30}{len(trailed)} (PnL={tpnl:+.2f}, WR={twr:.1f}%)"
            )
            print(
                f"  {'Trailing yok':<30}{len(not_trailed)} (PnL={npnl:+.2f}, WR={nwr:.1f}%)"
            )
        ...
        for i, t in enumerate(trades[-10:]):
            ...
            print(
                f"  {i+1:<4}{t['side']:<7}{t['entry_price']:<11.2f}{t.get('exit_price',0):<11.2f}{t['pnl']:<+10.2f}{t['rr']:<+8.2f}{t['result']:<6}{t.get('trailing_count',0):<6}{fvg_info}"
            )
    else:
        print("\n  Sinyal bulunamadi.")
    ...
    if trades:
        ...
        metrics.update(
            {"total_pnl": total_pnl, "wr": wr, "max_dd": dd_max, "profit_factor": pf}
        )
    return metrics
...
def main():
    ...
    if args.all:
        ...
        for m in sorted(results, key=lambda x: x["symbol"]):
            print(
                f"  {m['symbol']:<10} {m['total_trades']:<7} {m['total_pnl']:<+12.0f} {m['wr']:<5.1f}% {m['max_dd']:<6.1f}% {m['profit_factor']:<5.2f}"
            )
