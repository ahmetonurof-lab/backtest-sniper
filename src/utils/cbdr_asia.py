"""
cbdr_asia.py — ASIA_RANGE [01:00-05:00] CBDR threshold analysis.
...
def collect_daily_data(symbol: str):
    ...
    for sb in range(500, total_bars):
        if (sb - 500) % 5000 == 0:
            ...
            print(
                f"\r    [ASIA_RANGE] %{pct:.0f} ({sb}/{total_bars})", end="", flush=True
            )
    ...
    print(f"\r    [ASIA_RANGE] %100 ({total_bars}/{total_bars})", flush=True)
    daily_rows = []
...
def main():
    ...
    print("=" * 100)
    print(
        f"  CBDR ESIK ANALIZI — {SESSION_NAME} [{SESSION_HOURS['start']:02d}:00-{SESSION_HOURS['end']:02d}:00]"
    )
    print("=" * 100)
    ...
    parquet_path = os.path.join(
        ...
        "..",
        "reports",
        f"trades_{SESSION_NAME.lower()}.parquet",
    )
    global _LOGGER
    ...
    eh = SESSION_HOURS["end"]
    lines.append(
        f"# CBDR Threshold Analysis — {SESSION_NAME} [{sh:02d}:00-{eh:02d}:00]"
    )
    lines.append("")
