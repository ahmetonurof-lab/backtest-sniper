...
def simulate_portfolio(df, label, filters):
    ...
    if filters.get("el_risk"):
        ...
        print(
            f"    Early London: {el_count} trade {cfg.EARLY_LONDON_RISK_MULT}x skalalandi"
        )
    ...
    daily = (
        dfa.groupby(pd.to_datetime(dfa["entry_time"]).dt.date)["final_pnl_usd"]
        .sum()
        .sort_index()
    )
    cum = daily.cumsum()
    ...
    if trough_idx is not None:
        ...
        if not rec.empty:
            rec_days = (
                (rec.index[0] - trough_idx).days
                if hasattr(rec.index[0], "__sub__")
                else None
            )
...
for sname in ["default", "real_cbdr", "asia_range"]:
    ...
    rf = simulate_portfolio(
        df, "+ Relative FVG", {"session_router": True, "rel_fvg": True}
    )
    el = simulate_portfolio(
        df, "+ EL 1.5x", {"session_router": True, "rel_fvg": True, "el_risk": True}
    )
    ...
    for r in [baseline, sr, rf, el]:
        print(
            f"  {r['label']:<30} {r['trades']:>7,} {r['wr']:>5.1f}% {r['pnl']:>+9,.0f} {r['max_dd_pct']:>6.2f}% {r['avg_r']:>5.2f}"
        )
