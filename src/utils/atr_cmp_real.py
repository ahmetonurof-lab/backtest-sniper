...
CACHE_PATH = os.path.join(
    os.path.dirname(__file__), "..", "reports", "_atr_cmp_cache.json"
)
...
def main():
    ...
    for sym in coins_to_run:
        ...
        cache[sym] = stats
        print(
            f"{stats['analyzed']:,} bar | fake={stats['avg_fake']:.4f} real={stats['avg_real']:.4f} ratio={stats['avg_ratio']:.4f}",
            flush=True,
        )
    ...
    lines.append(
        "| Coin | "
        + " | ".join(
            str(y) for y in sorted({k for s in coin_stats for k in s["yearly"]})
        )
        + " |"
    )
    lines.append(
        "|------|"
        + "|".join("---" for _ in sorted({k for s in coin_stats for k in s["yearly"]}))
        + "|"
    )
    ...
    if btc_stats:
        ...
        lines.append(
            f"- BTC bazlı önerilen MULT: **{btc_mult:.4f}** (FVG 10.0 / Real ATR {btc_stats['avg_real']:.2f})\n"
        )
        lines.append(
            f"- Mevcut MULT 0.12 ile karşılaştırma: {'DÜŞÜK' if btc_mult < 0.12 else 'YÜKSEK' if btc_mult > 0.12 else 'AYNI'} ({abs(btc_mult-0.12)*100/0.12:.1f}% fark)\n"
        )
        lines.append(
            f"- Öneri: MULT = {btc_mult:.4f} (BTC anchor ile), veya coin bazlı override ile esneklik\n"
        )
    ...
    lines.append("")
    lines.append(
        f"- {sum(1 for _,m in mults if abs(m-CURRENT_MULT)/CURRENT_MULT>0.2)}/{len(mults)} coin mevcut MULT'tan %20'den fazla sapıyor.\n"
    )
    ...
    for s in coin_stats:
        ...
        if isinstance(old_r, str):
            ...
            lines.append(
                f"| {sym:<8} | {old_r:<20.2f} | {s['avg_ratio']:.4f} | {diff:+.4f} |"
            )
