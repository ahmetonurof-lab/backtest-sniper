...
"""
# ruff: noqa: E402
import csv, functools, math, os, sys, time, json
...
def load_data(path):
    ...
    with open(path, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            bars.append(Bar(index=len(bars), open=float(row["open"]), high=float(row["high"]),
...
def run_backtest(sym):
    ...
    for sb in range(500, total_bars):
        ...
        if rsm.can_trigger() and not active:
            ...
            # ── YENI: FVG expiry ──
            if tf is not None and (cur.index - tf.bar_index) > cfg.GLOBAL_FVG_EXPIRY_BARS:
                rsm.reset()
                continue
            ...
            cbdr_w = ((ss.cbdr_body_high - ss.cbdr_body_low) / ss.cbdr_body_low * 100) if ss.cbdr_body_low > 0 and not math.isinf(ss.cbdr_body_low) else None
            ...
            active.append({"entry_bar": sb, "entry_price": ep, "sl": sl, "tp": tp,
                           ...
                           "day_key": entry_day, "entry_hour": h,
                           "cbdr_mult": cbdr_mult, "el_mult": el_mult})
            rsm.reset()
        ...
        if active and cur.is_closed:
            ...
            cfvgs = detect_fvgs(tc, lookback=min(50, len(tc)), timeframe="15m", min_fvg_size=min_fvg_sz)
            for t in active:
                if t.get("closed"):
                    continue
                ...
                if upd:
                    t["sl"] = csl; t["tp"] = ctp; t["trailing_count"] = t.get("trailing_count", 0) + ltc
        ...
        for t in active:
            ...
            if t["side"] == "long":
                if cur.low <= t["sl"]:
                    t["exit_price"] = t["sl"]; t["exit_bar"] = sb; t["result"] = "SL"; t["closed"] = True; ex = True
                elif cur.high >= t["tp"]:
                    t["exit_price"] = t["tp"]; t["exit_bar"] = sb; t["result"] = "TP"; t["closed"] = True; ex = True
            else:
                if cur.high >= t["sl"]:
                    t["exit_price"] = t["sl"]; t["exit_bar"] = sb; t["result"] = "SL"; t["closed"] = True; ex = True
                elif cur.low <= t["tp"]:
            ...
            if ex:
                ...
                trade_records.append({"result": t["result"], "pnl": t["pnl"], "hour": t.get("entry_hour")})
            else:
                sa.append(t)
        active = sa
    ...
    return {"sym": sym, "trades": n, "wins": wins, "be": be, "losses": losses,
            "wr": round(wr, 1), "pnl": round(total_pnl, 2), "max_dd_pct": round(max_dd_pct, 2)}
...
def main():
    ...
    print(f"{'='*100}")
    print(f"  {'Coin':<10} {'Trades':>7} {'WIN':>6} {'BE':>5} {'LOSS':>6} {'WR%':>6} {'PnL':>10} {'MaxDD%':>8}")
    print(f"  {'-'*60}")
    for r in results:
        print(f"  {r['sym']:<10} {r['trades']:>7} {r['wins']:>6} {r['be']:>5} {r['losses']:>6} "
    ...
    print(f"  {'-'*60}")
    print(f"  {'TOPLAM':<10} {total_t:>7} {'-':>6} {'-':>5} {'-':>6} {avg_wr:>5.1f}% {total_p:>+9,.0f}")
