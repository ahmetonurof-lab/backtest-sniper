...
def find_sweeps(bars_15m, lookback=48):
    ...
    for i in range(lookback, len(bars_15m)):
        ...
        if recent["high"] > high_prev:
            sweeps.append({"idx": i, "direction": "above", "level": high_prev,
                           "timestamp": recent["timestamp"]})
        if recent["low"] < low_prev:
            sweeps.append({"idx": i, "direction": "below", "level": low_prev,
                           "timestamp": recent["timestamp"]})
    return sweeps
    ...
    for fvg in fvgs:
        ...
        for j in range(start_idx, min(start_idx + 200, len(bars))):
            ...
            if fvg["direction"] == "bullish":
                ...
                penetration = (fvg["bottom"] - b["low"]) / fvg["size"] if fvg["size"] > 0 else 0
            else:
                in_fvg = fvg["bottom"] <= b["close"] <= fvg["top"]
                below_fvg = b["high"] > fvg["top"]
                above_fvg = b["low"] < fvg["bottom"]
                penetration = (b["high"] - fvg["top"]) / fvg["size"] if fvg["size"] > 0 else 0
...
def main():
    ...
    for sym in all_symbols:
        ...
        avg_pen = np.mean([r["max_penetration_pct"] for r in life if r["violated"]]) if n_violated else 0
        ...
        # Print
        print(f"    Dolan:     {n_filled:>6} (%{pct_filled:5.1f}) | Ort. {avg_fill:5.1f} bar")
        print(f"    Ihlal:     {n_violated:>6} (%{pct_violated:5.1f}) | Ihlal sirasinda FVG'nin %%{avg_pen:.1f}'i delinmis")
        print(f"    Acik:      {n_open:>6} (%{n_open/len(life)*100:5.1f})" if life else "")
        print(f"    Sweep:     {len(sweeps):>6} adet | Toparlanma: %{pct_sw_rec:5.1f} | Ort. {avg_rec:5.1f} bar")

        # DNA profiling
        # fvg_expiry_bars: FVG'lerin %90'inin doldugu bar sayisi
        if fill_bars:
            sorted_bars = sorted(fill_bars)
            expiry = sorted_bars[int(len(sorted_bars) * 0.9)] if len(sorted_bars) > 10 else sorted_bars[-1]
        else:
            expiry = 0
        ...
        print(f"    🧬 DNA: expiry={expiry} bar, tol=%{violation_tolerance:.1f}, rec={avg_rec:.0f} bar")

    # ── SUMMARY ──
    print("\n" + "=" * 100)
    print("  DNA KARSILASTIRMA TABLOSU")
    print("=" * 100)
    print(f"  {'Coin':<10} {'Expiry':>7} {'Dolma':>7} {'Ihlal%':>7} {'Tolerans':>9} {'SweepRec':>9} {'Rec%':>6}")
    print(f"  {'-'*55}")
    for sym in cfg.SYMBOLS:
        d = dna_data.get(sym)
        if d:
            print(f"  {sym:<10} {d['fvg_expiry_bars']:>7} {d['avg_fill_bars']:>6.0f}b {d['violation_pct']:>6.1f}% {d['violation_tolerance']:>8.1f}% {d['sweep_recovery_bars']:>8.0f}b {d['sweep_recovery_pct']:>5.1f}%")
