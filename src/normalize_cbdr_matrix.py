...
REPORTS = {
    ...
    "ASIA_RANGE": os.path.join(BACKTEST_DIR, "..", "reports", "cbdr_asia_range_report.md"),
    "REAL_CBDR": os.path.join(BACKTEST_DIR, "..", "reports", "cbdr_real_cbdr_report.md"),
}
...
def modify_config(report_data):
    ...
    for session, fpath in SESSION_MAP.items():
        report = parse_report(os.path.join(os.path.dirname(BACKTEST_DIR), "reports", fpath))
        if not report:
