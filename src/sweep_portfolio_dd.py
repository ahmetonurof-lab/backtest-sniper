...
def calmar_ratio(daily_series):
    ...
    total_days = (daily_series.index[-1] - daily_series.index[0]).days if hasattr(daily_series.index, '__getitem__') else len(daily_series)
...
for mult in SWEEP + [round(1.0 + i * 0.5, 1) for i in [0,1,2,3,4,6,8]]:
    ...
    print(f"  {mult:<6.1f} {tp:>+10,.0f} {calmar:>7.2f}  {dd_pct:>6.2f}% {rec_str:>7} {rec_date:>12} {ratio:>6.1f} {el_pnl:>+10,.0f}")
...
print()
print(f"  {'Mult':<6} {'Calmar':>8} {'MaxDD%':>8} {'RecGun':>7} {'Peak':>12} {'Trough':>12} {'RecDate':>12} {'PF_Tum':>7} {'PF_EL':>7}")
print(f"  {'-'*85}")
...
for mult in [round(0.5+i*0.5,1) for i in range(10)]:
    ...
    rec_date = rec_d.strftime("%Y-%m-%d") if rec_d is not None else "-"
    print(f"  {mult:<6.1f} {calmar:>7.2f}  {dd_pct:>6.2f}% {rec_str:>7} {peak_s:>12} {trough_s:>12} {rec_date:>12} {pf_all:>6.2f} {pf_el:>6.2f}")
    df_s = df.copy()
    df_s.loc[df_s["is_el"], "final_pnl_usd"] *= mult
    daily = df_s.groupby("date")["final_pnl_usd"].sum().sort_index()
    tp = daily.sum()
    ...
    ratio = tp / dd_val if dd_val else 0
    el_pnl = df_s[df_s["is_el"]]["final_pnl_usd"].sum()
    print(f"  {mult:<6.1f} {tp:>+10,.0f} {dd_val:>10,.0f} {dd_pct:>9.2f}% {ratio:>6.1f} {el_pnl:>+10,.0f}")
...
print("=" * 100)
print()
print(f"  {'Coin':<10} {'Trades':>7} {'ToplamPnL':>10} {'WR%':>6} {'MaxDD$':>9} {'MaxDD%':>8} {'StdDev':>8} {'Sharpe':>7}")
print(f"  {'-'*65}")
...
for sym in sorted(df["symbol"].unique()):
    ...
    print(f"  {sym:<10} {n:>7} {tp:>+10,.0f} {wr:>5.1f}% {dd_val:>9,.0f} {dd_pct:>7.2f}% {std:>7.2f} {sharpe:>6.2f}")

print()
print(f"  PORTFOY TOPLAMI:")
print(f"  {'TOPLAM':<10} {len(df):>7} {daily_pnl.sum():>+10,.0f} {'-':>6} {port_dd_val:>9,.0f} {port_dd_pct:>7.2f}% {daily_pnl.std():>7.2f} {daily_pnl.mean()/daily_pnl.std()*(365**0.5) if daily_pnl.std() else 0:>6.2f}")
